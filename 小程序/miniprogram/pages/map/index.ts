...
function filterPoints(type: string): GuidePoint[] {
  if (type === '全部') return allPoints
...
Page({
  ...
  onKeywordInput(event: WechatMiniprogram.Input): void {
  ...
  onMarkerTap(event: WechatMiniprogram.CustomEvent<{ markerId: number }>): void {
  ...
  onPointTap(event: WechatMiniprogram.TouchEvent): void {
  ...
  onDetailTap(): void {
    const point = this.data.selectedPoint as GuidePoint | null
    if (!point) return
    if (point.type === '特色产品') {
