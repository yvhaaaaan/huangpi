...
| 关联前端 | `frontend/miniprogram` |
| 核心原则 | 一个小程序、一套后端、登录后由后端返回角色并进入对应前端 |

## 0. 当前实施状态

截至 2026-07-15，Java Spring Boot 后端 MVP 已建立并通过自动化端到端测试。

| 模块 | 状态 | 已实现范围 |
| --- | --- | --- |
| 认证与权限 | 已实现 | 微信登录（开发环境 mock / 生产环境 `jscode2session`）、账号密码登录、JWT、会话校验、退出失效、三角色接口隔离。 |
| 特色产品 | 已实现 | 产品品类、公开产品列表/详情、油茶与丝苗米同级重点配置。 |
| 商家端 | 已实现 | 工作台、资料保存/提交、产品新增/编辑/提交/下架、消息、待办；不包含活动管理。 |
| 政府端 | 已实现 | 工作台、审核列表/详情、通过、驳回、下架、商家列表、品类维护、审计日志。 |
| 用户文旅能力 | 已实现 | 首页聚合、搜索、地图点位、推荐路线、平台活动列表/详情、活动报名、我的报名、收藏/取消收藏。 |
| 文件能力 | 已实现 | JPG/PNG/WebP 上传、5 MB 限制、文件信息、权限校验、逻辑删除、本地开发存储。 |
| 自动化验证 | 已实现 | 覆盖商家提交、政府审核、用户查看、地图路线、活动报名、重复报名、收藏、角色越权和退出失效。 |
| 平台内容维护 | 后续 | 政府端维护活动、地图点位、路线、轮播图和通用内容的写接口。 |
| 生产部署 | 后续 | MySQL 实例、真实微信凭据、HTTPS 合法域名、对象存储/CDN、监控告警。 |
